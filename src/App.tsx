...
import { ref, onValue, push, set, serverTimestamp, off, query, orderByChild, startAt } from 'firebase/database';
...
export default function App() {
  ...
  // On utilise un Set local pour se souvenir des messages qu'on a envoyés
  // Cela évite de stocker un ID utilisateur dans Firebase (respect de la vie privée)
  ...
  useEffect(() => {
    ...
    // On ne récupère que les messages de la dernière heure (3600000 ms)
    ...
    const unsubscribe = onValue(messagesRef, (snapshot) => {
      ...
      if (data) {
        ...
        Object.entries(data).forEach(([key, val]: [string, any]) => {
          // Double vérification côté client pour le TTL d'une heure
    ...
    return () => {
      // Nettoyage de l'écouteur lorsqu'on quitte le salon
  ...
  const generateCode = () => {
    // Génération cryptographiquement aléatoire
    const array = new Uint32Array(1);
    window.crypto.getRandomValues(array);
    // On s'assure d'avoir 6 chiffres (entre 100000 et 999999)
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    if (inputText.trim().length > 0) {
      ...
      // On mémorise la clé pour identifier ce message comme le nôtre dans l'UI
      if (newMsgRef.key) {
      ...
      set(newMsgRef, {
        ...
        console.error("Erreur d'envoi (vérifiez votre configuration Firebase):", err);
      });
        ...
        <main className="flex-1 flex flex-col relative">
          <AnimatePresence mode="wait">
            {!inRoom ? (
              ...
              >
                {/* Room Info */}
                <div className="bg-white dark:bg-neutral-900/50 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm dark:shadow-none">
                  <div>
                  ...
                  >
                    {copiedCode ? <Check className="w-4 h-4 text-green-500 dark:text-green-400" /> : <Copy className="w-4 h-4" />}
                    {copiedCode ? 'Copied!' : 'Copy Code'}
                  </button>
                </div>

                {/* Messages Area */}
                <div className="flex-1 bg-white dark:bg-neutral-900/30 border border-neutral-200 dark:border-neutral-800/50 rounded-2xl p-4 mb-6 overflow-y-auto flex flex-col gap-4 shadow-sm dark:shadow-none">
                  {messages.length === 0 ? (
                      ...
                      <motion.div
                        ...
                        className={cn(
                          ...
                          msg.sender === 'me' 
                            ...
                            : "bg-neutral-100 text-neutral-900 dark:bg-neutral-800 dark:text-neutral-100 self-start rounded-bl-sm shadow-sm dark:shadow-none"
                        )}
                      >
                ...
                <form onSubmit={handleSendMessage} className="relative">
                  <textarea
                    ...
                    className="w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl py-4 pl-5 pr-16 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all min-h-[60px] max-h-[200px] text-neutral-900 dark:text-white shadow-sm dark:shadow-none"
                    rows={1}
                  />
                  <button
                    type="submit"
